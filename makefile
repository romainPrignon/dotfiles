...
test-qa: ## run comprehensive QA tests in Docker - make test-qa version=trixie (default: trixie)
	@echo "=== Running Comprehensive QA Tests in Docker ==="
	@echo ""
	@echo "1. Testing Docker Container Setup..."
	@docker run --rm romainprignon/dotfiles:${version} bash -c " \
		...
		echo ''"
	@echo ""
	@echo "2. Testing Essential Tools Installation..."
	@docker run --rm romainprignon/dotfiles:${version} bash -c " \
		for tool in git curl wget make vim zsh htop jq sqlite3 ffmpeg tree xclip shellcheck; do \
	...
	@echo ""
	@echo "3. Testing Repository Structure..."
	@docker run --rm romainprignon/dotfiles:${version} bash -c " \
		cd /home/romainprignon/workspace/romainprignon/dotfiles && \
		echo '   - Total files:' \$$(find . -type f | wc -l) && \
		echo '   - Scripts:' \$$(find scripts -type f 2>/dev/null | wc -l) && \
		echo '   - Dotfiles:' \$$(ls -1 .* 2>/dev/null | grep -v '^\\.\\.\?$$' | wc -l)"
	@echo ""
	@echo "4. Running make install..."
	@docker run --rm romainprignon/dotfiles:${version} bash -c " \
		timeout 180 make install > /tmp/install.log 2>&1; \
		if [ \$$? -eq 0 ] || [ \$$? -eq 124 ]; then \
	...
	@echo ""
	@echo "5. Verifying Post-Installation State..."
	@docker run --rm romainprignon/dotfiles:${version} bash -c " \
		make install > /dev/null 2>&1; \
		for tool in git curl wget make vim zsh htop jq sqlite3; do \
		  if command -v \$$tool &>/dev/null; then \
		    version=\$$((\$$tool --version 2>&1 || \$$tool -version 2>&1 || echo 'installed') | head -1); \
		    echo \"   ✓ \$$tool - \$$version\"; \
		  else \
		    echo \"   ✗ \$$tool - NOT FOUND\"; \
		  fi; \
